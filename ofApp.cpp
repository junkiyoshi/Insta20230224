...
void ofApp::setup() {
	...
	for (auto& location : this->location_list) {
 	...
 	for (int i = 0; i < 25; i++) {
		...
		this->actor_list.back()->setColor(ofColor(0));
	}
...
void ofApp::update() {
	...
	for (int i = 0; i < this->actor_list.size(); i++) {
		...
		this->mesh.addVertex(this->actor_list[i]->getLocation());
	...
	for (int i = 0; i < this->mesh.getNumVertices(); i++) {
		...
		this->mesh.addColor(ofColor(255, ofMap(min_distance_list[i], 12, 10, 0, 255)));
	}
}
...
void ofApp::draw() {
	...
	for (auto& actor : this->actor_list) {
		...
		ofDrawSphere(actor->getLocation(), 0.5);
	}
